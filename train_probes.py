...
from train_gpt import get_batch, eval_loss, device 

def train_standalone_probe(model, site, train_data, eval_batches, config,
                           probe_dir, run_name,
  ...
  if os.path.exists(probe_path):
    ...
    if probe_ckpt['step'] == max_steps:
      ...
      print(f"Trained probe for run_name: '{run_name}' already available @ {probe_path}")
  ...
  def eval_probe():
    ...
    for x, y in eval_batches:
      logits = probe(model.forward_repr_at_site(site, x))
      B, T, _ = logits.shape
      losses.append(
  ...
  for step in range(max_steps+1):
    ...
    with torch.no_grad():
      repr_ = model.forward_repr_at_site(site, x)
    logits = probe(repr_)
    B, T, _ = logits.shape
...
def train_xl2xs_probe(xs_model, xl_model, site, head, train_data, eval_batches,
                      config, traj_probe_dir, traj_step_pt, steps=3000, eval_every=250,
  ...
  print(f"Starting xl({xl_n_embed})2xs({xs_n_embed}) for " \
  ...
  for step in range(steps+1):
    ...
    # forward
    with torch.no_grad():
      xl2xs = head(xl_model.forward_repr_at_site(site, x))
    logits = probe(xl2xs)
    B, T, _ = logits.shape
    loss = F.cross_entropy(logits.view(B*T, V), y.view(B*T))
